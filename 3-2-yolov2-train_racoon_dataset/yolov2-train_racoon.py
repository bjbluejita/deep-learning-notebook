...
@Package 
@author: ly
@date Date: 2019年03月11日 11:08
@Description: 
@URL: https://nbviewer.jupyter.org/github/erhwenkuo/deep-learning-with-keras-notebooks/blob/master/3.2-yolov2-train_racoon_dataset.ipynb
@version: V1.0
'''
import  os
import sys
sys.path.append( 'F:/workspace/Tensorflow/src/deep-learning-with-keras-notebooks/3-2-yolov2-train_racoon_dataset' )
import random
...
# 資料集圖像檔目錄
IMAGE_PATH = os.path.join( DATA_SET_PATH, 'images' )
...
train_image_folder = IMAGE_PATH
train_annot_folder = ANNOTATIONS_PATH
valid_image_folder = IMAGE_PATH
...
# Layer 6
x = Conv2D( 256, (3,3),  padding='same', name='conv_6', use_bias=False )( x )
...
# Layer 9
x = Conv2D( 512, (3,3),  padding='same', name='conv_9', use_bias=False )( x )
...
# Layer 12
x = Conv2D( 256, (1,1),  padding='same', name='conv_12', use_bias=False )( x )
...
# Layer 15
x = Conv2D( 512, (1,1),  padding='same', name='conv_15', use_bias=False )( x )
...
for i in range( 1, nb_conv+1 ):
    ...
    if i < nb_conv:
        ...
        gama = weight_reader.read_bytes( size )
        mean = weight_reader.read_bytes( size )
        var  = weight_reader.read_bytes( size )
        weights = norm_layer.set_weights( [ gama, beta, mean, var ] )
...
def custom_loss( y_true, y_pred ):
    ...
    :param y_true:
    :param y_pred: shape[2 13 13 5 6], 2:训练的图像个数，与BATCH_SIZE一致
                                       13,13：每张图像的cell维度，将一张图片划分为13X13 个格子（CELL）
                                       5: 每个CELL要预测的ANCHORS个数，现在定为5个
                                       6：每个ANCHORS要预测对象的内容，包括：对象的位置：X,Y,W,H
                                                                             对象的信心指数：CONF
                                                                             对象的类别： CLASS
                                          共6位数据
    :return: loss
    '''
    #print( 'custom_loss y_true.shape:{}  y_pred.shape:{}\n'.format( tf.shape(y_true) , tf.shape(y_pred ) ) )
    y_pred = tf.Print( y_pred, [tf.shape( y_pred ) ], message='y_pred shape:',summarize=1000 )
    y_true = tf.Print( y_true, [tf.shape( y_true ) ], message='y_true shape:',summarize=1000 )

    mask_shape = tf.shape( y_true )[:4]
    mask_shape = tf.Print( mask_shape, [mask_shape], message='mask_shape', summarize=1000 )
    '''
    tf.tile:用来对张量(Tensor)进行扩展的，其特点是对当前张量内的数据进行一定规则的复制
    '''
    ...
    #tf.expand_dims: 维度增加一维,axis=0 最前增加1维， axis=-1最后增加1维
...
checkpoint = ModelCheckpoint( 'weight_racoon.h5',
                              monitor='val_loss',
                              verbose=0,
                              save_best_only=True,
                              mode='min',
...
#model = load_model( 'model.h5',  custom_objects={'tf':tf, 'custom_loss':custom_loss } )
model.load_weights( 'C:/Users/Administrator/Downloads/weights_reccoon.h5' )
# 產生一個Dummy的標籤輸入
...
img_filepath = 'F:/workspace/Tensorflow/src/deep-learning-with-keras-notebooks/3-2-yolov2-train_racoon_dataset/data/racoon/images/raccoon-34.jpg'
#img_filepath = 'C:/Users/Administrator/Downloads/test3.jpg'
print(img_filepath)
...
video_inp = os.path.join( 'C:/Users/Administrator/Downloads/t0029z1d676.mp4' )
...
# 取得影像的基本資訊
nb_frame = int(  video_reader.get( cv2.CAP_PROP_FRAME_COUNT ) ) # 總共有多少frames
...
detect_num = 1
#netout = np.zeros( [1, 13, 13, 5, 6], dtype=np.float32 )
...
while( True ):
    ...
    if detect_num % 20 == 0:
        ...
        boxes = decode_netout(netout[0],
                              ...
                              anchors=ANCHORS,
                              nb_class=CLASS)
    frame = draw_bgr_image_boxes(frame, boxes, labels=LABELS)
