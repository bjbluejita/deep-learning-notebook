...
class CycleGAN:
    ...
    def model(self):
        ...
        #X->Y
        fake_y = self.G( x )
        G_gen_loss = self.generator_loss( self.D_Y, fake_y, use_lsgan=self.use_lsgan )
        ...
        tf.summary.scalar( 'loss/G', G_gen_loss )
    ...
    def optimize(self, G_loss, D_Y_loss, F_loss, D_X_loss ):
        def make_optimizer( loss, variables, name='Adam' ):
            ...
            start_dacay_step = 100000
            dacay_steps = 100000
            beta1 = self.beta1
            learning_rate = (
                tf.where(
                    tf.greater_equal( global_step, start_dacay_step ),
                    tf.train.polynomial_decay( starter_learning_rate, global_step-start_dacay_step,
                                               decay_steps=dacay_steps, end_learning_rate=end_learning_rate,
                                               power=1.0 ),
        ...
        F_optimizeer = make_optimizer( F_loss, self.F.variables, name='Adam_F' )
        D_X_optimizer = make_optimizer( D_X_loss, self.D_X.variables, name='Adam_D_X' )

        with tf.control_dependencies( [ G_optimizer, D_Y_optimizer, F_optimizeer, D_X_optimizer ] ):
