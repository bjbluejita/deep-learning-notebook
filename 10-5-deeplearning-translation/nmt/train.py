...
@Package 
@author: ly
@date Date: 2019年08月12日 14:42
@Description: For training NMT models.
@URL: https://github.com/tensorflow/nmt/blob/master/nmt/train.py
@version: V1.0
'''
...
def print_step_info( prefix, global_step, info, result_summary, log_f ):
    ...
    utils.print_out(  "%s: step:%d  lr:%g step-time:%.2fs wps:%.2fK ppl:%.2f gN:%.2f %s, %s" %
...
def process_stats( stats, info, global_step, steps_per_stats, log_f ):
    ...
    if math.isnan( train_ppl ) or math.isinf( train_ppl ) or train_ppl > 1e20:
        utils.print( ' ☆ step %d overflow, stop early' % global_step, log_f )
        is_overflow = True
...
def train( hparams, scope=None, target_session="" ):
    ...
    #preload data from sample decoding
    dev_src_file = '%s.%s' % ( hparams.dev_prefix, hparams.src )
    dev_tgt_file = '%s.%s' % ( hparams.dev_prefix, hparams.tgt )
    ...
    utils.print_out( '# log_file=%s' % log_file )

    #TensorFlow model
    config_proto = utils.get_config_proto(
        log_device_placement=log_device_placement,
        num_intra_threads=hparams.num_intra_threads,
        num_inter_threads=hparams.num_intra_threads
    )
...
#       ## Run a step ###
        ...
        if global_step - last_stats_step >= steps_per_stats:
            last_stats_step = global_step
            isOverflow = process_stats(
                stats, info, global_step, steps_per_stats, log_f
            )
            print_step_info( "step_info", global_step, info, get_best_results( hparams ),
                             log_f )
            if isOverflow:
                utils.print_out( 'Over Flow !' )
                break
    ...
    ( result_summary, _, final_eval_metric ) = (
        run_full_eval(
    ...
    utils.print_out( '# Start evaluating saved best model ' )
    ...
    return final_eval_metric, global_step
    ...
    if decode:
        for metric in hparams.metrics:
            ...
            if save_on_best and scores[metric] > getattr( hparams, best_metric_label ):
                ...
                model.saver.save(
                    sess,
                    os.path.join( getattr( hparams, best_metric_label + '_dir'), 'translation.ckpt' ),
