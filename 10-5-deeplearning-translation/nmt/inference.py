...
@Package 
@author: ly
@date Date: 2019年08月09日 14:41
@Description: To perform inference on test set given a trained model.
@URL: https://github.com/tensorflow/nmt/blob/master/nmt/inference.py
@version: V1.0
'''
...
def _decode_inference_indices( model, sess, output_infer,
    ...
    utils.print_out( 'decoding to  output %s, num sents %s' %
    ...
    with codecs.getwriter( 'utf-8' )( tf.gfile.GFile( output_infer, mode='wb')) as trans_f:
        ...
        for decode_id in inference_indices:
            ...
            if infer_summary is not  None: # Attention models
                ...
                with tf.gfile.GFile( image_file, 'w' ) as img_f:
...
def single_worker_inference( sess,
    ...
    #Read data
    utils.print_out( 'load data' )
    infer_data = load_data( inference_input_file, hparams )

    with infer_model.graph.as_default():
        utils.print_out( 'run  initializer' )
        sess.run(
...
def multi_worker_inference(sess,
    ...
    #Read data
    infer_data = load_data( inference_output_file, hparams )
    ...
    nmt_utils.decode_and_evaluate(
        ...
        beam_width=hparams.beam_width,
        tgt_eos=hparams.eos,
        num_translations_per_input=hparams.num_translation_per_input,
        infer_mode=hparams.infer_mode
    )

    #Chanmg file name to indicate the file writing is completed.
    ...
    with codecs.getwriter( 'utf-8' )(
        tf.gfile.GFile( final_output_infer, 'wb')
    ) as final_f:
        for work_id in range( num_workers ):
            worker_infer_done = '%s_done_%d' % ( inference_output_file, work_id )
            while not tf.gfile.Exists( worker_infer_done ):
                utils.print_out( ' waiting job %d to complete.' % work_id )
                time.sleep( 10 )

            with codecs.getreader( 'utf-8' )(
                tf.gfile.GFile( worker_infer_done, 'rb' )
            ) as f:
