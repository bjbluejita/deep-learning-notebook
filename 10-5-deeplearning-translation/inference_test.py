...
@Package 
@author: ly
@date Date: 2019年08月12日 10:46
@Description: Tests for model inference."
@URL: https://github.com/tensorflow/nmt/blob/master/nmt/inference_test.py
@version: V1.0
'''
...
class InferenceTest( tf.test.TestCase ):
    ...
    def _createTestInferenceCheckpint(self, hparams, name ):
        ...
        with self.test_session( graph=infer_model.graph ) as sess:
            ...
            ckpt_path = loaded_model.saver.save(
                sess, os.path.join( out_dir, 'translation.ckpt'),
    ...
    def testBasicModelWithMultipleTranslations(self ):
        ...
        ckpt_path = self._createTestInferenceCheckpint( hparams, 'multi_basice_infer' )
        infer_file = 'nmt/testdata/test_infer_file'
        output_infer = os.path.join( hparams.out_dir, 'output_infer' )
        inference.inference( ckpt_path, infer_file, output_infer, hparams )
        with open( output_infer ) as f:
    ...
    def testAttentionModel(self):
        hparams = common_test_utils.create_test_hparams(
            ...
            use_residual=False,)
        ckpt_path = self._createTestInferenceCheckpint(hparams, "attention_infer")
        infer_file = "nmt/testdata/test_infer_file"
        output_infer = os.path.join(hparams.out_dir, "output_infer")
        inference.inference(ckpt_path, infer_file, output_infer, hparams)
        with open(output_infer) as f:
    ...
    def testMultiWorkers(self):
        ...
        ckpt_path = self._createTestInferenceCheckpint(hparams, "multi_worker_infer")
        infer_file = "nmt/testdata/test_infer_file"
        output_infer = os.path.join(hparams.out_dir, "output_infer")
        inference.inference(
