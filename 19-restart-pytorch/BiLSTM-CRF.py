...
class BiLSTM_CRF( nn.Module ):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim ):
        ...
        self.work_embeds = nn.Embedding( vocab_size, embedding_dim )
    ...
    def _get_lstm_features(self, sentence ):
        self.hidden = self.init_hidden()
        embeds = self.work_embeds( sentence ).view( len( sentence ), 1, -1 )
        lstm_out, self.hidden = self.lstm( embeds, self.hidden )
        lstm_out = lstm_out.view( len(sentence), self.hidden_dim )
        lstm_feats = self.hidden2tag( lstm_out )
        return lstm_feats

    def _score_sentenc(self, feats, tags ):
        score = torch.zeros(1)
    ...
    def neg_log_likelihood(self, sentence, tags ):
        ...
        gold_score = self._score_sentenc( feats, tags )
...
word_to_idx = {}
for sentenc, tags in training_data:
    for word in sentenc:
