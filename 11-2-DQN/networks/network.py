...
@Package 
@author: ly
@date Date: 2019年11月18日 16:58
@Description: 
@URL: https://github.com/hzy46/Deep-Learning-21-Examples/blob/master/chapter_20/networks/network.py
@version: V1.0
'''
...
class Network( object ):
    ...
    def build_output_ops(self, input_layer, network_output_type,
                         ...
                         weights_initializer, bias_initializer, hidden_activation_fn,
        ...
        if network_output_type == 'normal':
            self.outputs, self.var['w_out'], self.var['b_out'] = \
               linear( input_layer, output_size, weights_initializer,
                       bias_initializer, output_activation_fn, trainable, name='out' )
        elif network_output_type == 'dueling':
            #Dueling Network
            assert len( value_hidden_size ) !=0 and len( advantage_hidden_sizes ) != 0

            layer = input_layer
            for idx, hidden_size in enumerate( value_hidden_size ):
                w_name, b_name = 'val_w_%d' % idx, 'val_b_%d' % idx

                layer, self.var[ w_name ], self.var[ b_name ] = \
                    linear( layer, hidden_size, weights_initializer,
                            bias_initializer, hidden_activation_fn, trainable, name='val_lin_%d' % idx )

            self.value, self.var[ 'val_w_out' ], self.var[ 'val_w_b' ] = \
                linear( layer, 1, weights_initializer,
                        bias_initializer, output_activation_fn, trainable, name='val_lin_out' )
            ...
            for idx, hidden_size in enumerate( advantage_hidden_sizes ):
                ...
                layer, self.var[ w_name ], self.var[ b_name ] = \
                    linear( layer, hidden_size, weights_initializer,
                            bias_initializer, hidden_activation_fn, trainable, name='adv_lin_%d' % idx )

            self.advantage, self.var[ 'adv_w_out' ], self.var[ 'adv_w_b' ] = \
                linear( layer, output_size, weights_initializer,
                        bias_initializer, output_activation_fn, trainable, name='adv_lin_out' )
