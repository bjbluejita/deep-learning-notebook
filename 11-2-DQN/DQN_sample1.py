...
@Package 
@author: ly
@date Date: 2019年11月25日 10:40
@Description: 
@URL: https://segmentfault.com/a/1190000018120424?utm_source=tag-newest
@version: V1.0
'''
...
class DeepQNetwork:
    ...
    def __init__(self, learning_rate=0.001, gamma=0.9, memory_size=5000 ):
        ...
        # 初始化成一个 6 X 6 的状态矩阵。nxn的主对角线为1，其余地方为0的数组
    ...
    def create_network(self):
        ...
        # 取出当前动作的得分，即当前state下采取action预计reward值。
        self.reward_action = tf.reduce_sum( tf.multiply( self.q_eval, self.action_input ), reduction_indices=1  )
        #self.q_target是下4帧的reward值，self.reward_action当前状态的rewar值,这两个值越接近越好
    ...
    def select_action(self, state_index ):
        ...
        if np.random.uniform() < self.epsilon:  #随机数小于epsilon（ 初始值0.1），随便挑一个action
    ...
    def experience_replay(self):
        ...
        for i in range( len( minibatch ) ):
            ...
            # 更新 Q 值。
            q_value = current_reward + self.gamma * np.max( q_next[0][i] )  #q_next[0][i]：下一阶段所能带来的收益

            #q_target：下一阶段的reward
        ...
        #print( 'global step:', self.learn_step_counter, ' cost:', cost )
    ...
    def pay(self ):
        ...
        for index in range( 5 ):
            ...
            print("#############################", "Agent 在", target_state, "结束行动", "#############################\n")
