...
@Package 
@author: ly
@date Date: 2019年05月29日 15:32
@Description: 
@URL: https://github.com/hzy46/Deep-Learning-21-Examples/blob/master/chapter_8/model.py
@version: V1.0
'''
...
class DCGAN( object ):
    def __init__(self, sess, input_height=108, input_width=108, crop=True,
        ...
        if self.dataset_name == 'mnist':
            ...
            imreadImg = utils.imread( self.data[0] )
            if len( imreadImg.shape ) >= 3:  #check if image is a non-grayscale image by checking channel number
                self.c_dim = utils.imread( self.data[0] ).shape[-1]
            else:
                self.c_dim = 1
        ...
        for epoch in range(config.epoch):
            ...
            for idx in range(0, batch_idxs):
                if config.dataset == 'mnist':
                    ...
                    if self.grayscale:
                        batch_images = np.arrray(batch).astype(np.float32)[:, :, :, None]
                    else:
    ...
    def discriminator(self, image, y=None, reuse=False ):
        with tf.variable_scope( 'discriminator' ) as scope:
            ...
            if not self.y_dim:
                ...
                h0 = ops.lrelu( ops.conv2d( x, self.c_dim + self.y_dim, name='d_h0_conv' ) )
                h0 = ops.conv_cond_concat( h0, yb )

                h1 = ops.lrelu( self.d_bn1( ops.conv2d( h0, self.df_dim+self.y_dim, name='d_h1_conv' ) ) )
    ...
    def generator(self, z, y=None ):
        with tf.variable_scope( 'generator' ) as scope:
            if not self.y_dim:
                ...
                h0 = ops.concat( [h0, y], 1 )

                h1 = tf.nn.relu(
                    self.g_bn1(
                        ops.linear( h0, self.gf_dim*2*s_h4*s_w4, 'g_h1_lin')
                    )
                )
                ...
                h2 = tf.nn.relu(
                    self.g_bn2(
                        ops.deconv2d( h1, [self.batch_size, s_h2, s_w2, self.gf_dim*2 ], name='g_h2' )
                    )
                )
                h2 = ops.conv_cond_concat( h2, yb )

                return tf.nn.sigmoid(
                    ops.deconv2d( h2, [self.batch_size, s_h, s_w, self.c_dim ], name='g_h3')
                )
    ...
    def sampler(self, z, y=None ):
        with tf.variable_scope( 'generator' ) as scope:
            ...
            if not self.y_dim:
                ...
                # project `z` and reshape
                h0 = tf.reshape(
                    ops.linear(z, self.gf_dim*8*s_h16*s_w16, 'g_h0_lin'),
                ...
                h0 = tf.nn.relu( self.g_bn0( ops.linear( z, self.gfc_dim, 'g_h0_lin' ), train=False ) )
                h0 = ops.concat( [ h0, y ], 1 )

                h1 = tf.nn.relu( self.g_bn1(
                    ops.linear( h0, self.gf_dim*2*s_h4*s_w4, 'g_h1_lin' ), train=False
                ))
                h1 = tf.reshape( h1, [ self.batch_size, s_h4, s_w4, self.gf_dim*2 ] )
                h1 = ops.conv_cond_concat( h1, yb )

                h2 = tf.nn.relu( self.g_bn2(
                    ops.deconv2d( h1, [ self.batch_size, s_h2, s_w2, self.gf_dim*2], name='g_h2'), train=False
                ))
                h2 = ops.conv_cond_concat( h2, yb )

                return tf.nn.sigmoid( ops.deconv2d( h2, [ self.batch_size, s_h, s_w, self.c_dim ], name='g_h3' ))
    ...
    def load(self, checkpoint_dir ):
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            print( '[*] Sucess to read{}'.format( ckpt_name ) )
            return True, counter
        else:
