...
def get_image_paths_and_labels( dataset, enable_filter=False, filter_size=1 ):
    '''
    "取得人臉圖像集的圖像路徑與標籤
    :param dataset:  (list<ImageClass>): 人臉類別(ImageClass)的列表與圖像路徑
    :param enabel_filter:  (boolean): 是否要過濾一些人臉的Identity
    :param filter_size: 人臉的圖像數要大於或等於某個數量
    :return: image_paths_flat (list[string])： 人臉類別圖像路徑列表
             labels_flat (list[string]): 人臉類別圖像的編碼
             labels_dict (dic{number:label}): 人臉類別圖像的編碼對應到人臉的Identity
    '''
    dataset_filter = []

    if enable_filter:
        for i in range( len( dataset ) ):
            if ( len( dataset[i] ) >= filter_size ):
                dataset_filter.append( dataset[i] )
    else:
    ...
    for i in range( len( dataset_filter ) ):
        image_paths_flat += dataset_filter[i].image_paths
        labels_flat += [i] * len( dataset_filter[i].image_paths )
        labels_dict[i] = dataset_filter[i].name

    return image_paths_flat, labels_flat, labels_dict

def load_model( model ):
    '''
    DESC: Check if the model is a model directory (containing a metagraph and a checkpoint file)
...
def get_model_filenames( model_dir ):
    ...
    if len( meta_files ) == 0:
        raise  ValueError( 'No meta file found in the model director (%s)' % model_dir )
    elif len( meta_files ) > 1:
        raise ValueError( 'There should not be more than one meta file in the model dir (%s' % model_dir )
