...
@Package 
@author: ly
@date Date: 2019年06月25日 14:32
@Description: 
@URL: https://github.com/hzy46/Deep-Learning-21-Examples/blob/master/chapter_12/model.py
@version: V1.0
'''
...
class CharRNN:
    ...
    def build_input( self ):
        with tf.name_scope( 'inputs' ):
            ...
            if self.use_embedding is False:
                self.lstm_inputs = tf.one_hot( self.inputs, self.num_class )
            else:
                embedding = tf.get_variable( 'embedding', [ self.num_class, self.embedding_size] )
    ...
    def build_loss(self ):
        with tf.name_scope( 'loss' ):
            y_one_hot = tf.one_hot( self.targets, self.num_class )
