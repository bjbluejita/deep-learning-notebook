...
@Package 
@author: ly
@date Date: 2019年08月05日 15:33
@Description: 
@URL: https://github.com/wavewangyue/tensorflow_seq2seq/blob/master/model_seq2seq_contrib.py
@version: V1.0
'''
...
class Seq2seq( object ):
    ...
    def __init__(self, config, w2i_target, useTeacherForcing=True, useAttention=True, useBeamSearch=1):
        ...
        with tf.variable_scope( 'encoder' ):
            ...
            encoder_inputs_emedded = tf.nn.embedding_lookup( encoder_embedding, self.seq_inputs )
        ...
        with tf.variable_scope( 'decoder' ):
            ...
            with tf.variable_scope( 'gru_cell' ):
                ...
                if useAttention:
                    if useBeamSearch > 1:
                        tiled_ecoder_outputs = tf.contrib.seq2seq.tile_batch( encoder_outputs, multiplier=useBeamSearch )
                        tiled_sequence_length = tf.contrib.seq2seq.tile_batch( self.seq_inputs_length, multiplier=useBeamSearch )
                        attention_mechanism = tf.contrib.seq2seq.BahdanauAttention( num_units=config.hidden_dim, memory=tiled_ecoder_outputs,
            ...
            decoder_outputs, decoder_state, final_sequence_length = tf.contrib.seq2seq.dynamic_decode( decoder, maximum_iterations=tf.reduce_max( self.seq_targets_length ) )
