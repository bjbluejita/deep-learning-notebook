...
@Package 
@author: ly
@date Date: 2019年08月06日 15:15
@Description: 
@URL: https://github.com/wavewangyue/tensorflow_seq2seq/blob/master/model_seq2seq.py
@version: V1.0
'''
...
class Seq2seq( object ):
    ...
    def __init__(self, config, w2i_target, useTeacherForcing=True, useAttention=True, useBeamSearch=1 ):
        ...
        with tf.variable_scope( 'encoder' ):
            ...
            ( ( encoder_fw_ouputs, encoder_bw_outputs ), ( encoder_fw_final_state, encoder_bw_final_state) ) = tf.nn.bidirectional_dynamic_rnn(
            ...
            encoder_outputs = tf.add( encoder_fw_ouputs, encoder_bw_outputs )

        with tf.variable_scope( 'decoder' ):
            decoder_embedding = tf.Variable( tf.random_uniform( [ config.target_vocab_size, config.embedding_dim ]), dtype=tf.float32, name='decoder_emedding' )
            ...
            def loop_fn( time, previous_output, previous_state, previous_loop_state ):
                if previous_state is None:   # time step == 0
                    ...
                    elemnets_finished = ( time >= self.seq_targets_length )
                    finished = tf.reduce_all( elemnets_finished ) #Computes the "logical and"
                    ...
                    return ( elemnets_finished, input, state, output, loop_state )
