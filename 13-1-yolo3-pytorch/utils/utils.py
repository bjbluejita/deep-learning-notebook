...
'''
@File    :   utils.py
@Time    :   2020/02/24 14:02:25
@Author  :   LY 
@Version :   1.0
@URL     :   https://github.com/liuyuemaicha/PyTorch-YOLOv3/blob/master/utils/utils.py
...
def compute_ap( recall, precision ):
    ...
    # first append sentimel values at the end
    ...
    # where X axis ( recall ) change value
    i = np.where( mrec[ 1: ] != mrec[ :-1 ] )[0]

    # and sum( \Delta recal ) * prec
    ap = np.sum( ( mrec[ i + 1 ] - mrec[i] ) * mpre[ i + 1 ] )

    return ap


def get_batch_statistics( ouputs, targets, iou_threshold ):
    '''
    Compute true positives, predicted scores and predicted labels per sample
    '''
    batch_metrics = []

    for sample_i in range( len( ouputs ) ):
        if ouputs[ sample_i ] is None:
            continue

        # output: (pred_boxes_num, 7) 7 =》x,y,w,h,conf,class_conf,class_pred
        ouputs = ouputs[ sample_i ]

        pred_boxes = ouputs[ :, :4 ]  # 预测框的x,y,w,h
        pred_scores = ouputs[ :, 4 ]   # 预测框的置信度
        pred_labels = ouputs[ :, -1 ]  # 预测框的类别label
...
def bbox_iou( box1, box2, x1y1x2y2=True ):
    ...
    # Union Area
    b1_area = ( b1_x2 - b1_x2 + 1 ) * ( b1_y2 - b1_y1 + 1 )
...
def non_max_suppression( prediction, conf_thres=0.5, nms_thres=0.4 ):
    ...
    for image_i, image_pred in enumerate( prediction ):
        ...
        while detections.size( 0 ):
            ...
            detections[ 0, :4 ] = ( weights * detections[ invalid, :4 ]).sum( 0 ) / weights.sum( 0 )
            # 保存当前序列中最终识别的预测框
...
def build_targets( pred_boxes, pred_cls, target, anchors, ignore_thres ):
    ...
    noobj_mask[ b, best_n, gj, gi ] = 0 # 对目标实体框中心点所在的单元网格，其最优anchor设置为0
