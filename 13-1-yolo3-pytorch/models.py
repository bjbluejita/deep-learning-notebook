...
'''
@File    :   models.py
@Time    :   2020/02/24 13:58:35
@Author  :   LY 
@Version :   1.0
@URL     :   https://github.com/liuyuemaicha/PyTorch-YOLOv3/blob/master/models.py
...
def create_modules( module_defs ):
    ...
    for module_i, module_def in enumerate( module_defs ):
        ...
        if module_def[ 'type' ] == 'convolutional':
            bn = bool( module_def[ 'batch_normalize' ] )
            ...
            maxpool = nn.MaxPool2d( kernel_siz=kernel_size, stride=stride, padding=int( ( kernel_size - 1) // 2 ) )
...
class YOLOLayer( nn.Module ):
    ...
    def compute_grid_offset( self, grid_size, cuda=True ):
        ...
        # Calculate offset for each grid
    ...
    def forward( self, x, targets=None, img_dim=None ):
        ...
        # Get outputs
        # 这里的prediction是初步的所有预测，在grid_size*grid_size个网格中，它表示每个网格都会
        # 有num_anchor（3）个anchor框
        ...
        pred_cls = torch.sigmoid( prediction[ ..., 5: ] )   # 类别

        # if grid size does not match current we compute new offset
        if grid_size != self.grid_size:
            self.compute_grid_offset( grid_size, cuda=x.is_cuda )

        # Add offset and scale with anchors
        pred_boxes = FloatTensor( prediction[ ..., :4 ].shape )
        # 针对每个网格的偏移量，每个网格的单位长度为1，而预测的中心点（x，y）是归一化的（0，1之间），
        # 所以可以直接相加
...
class Darknet( nn.Module ):
    ...
    def save_darknet_weights( self, path, cutoff=-1 ):
        ...
        for i, ( module_def, module ) in enumerate( zip( self.module_defs[ :cutoff ], self.module_list[ :cutoff ] ) ):
            if module_def[ 'type' ] == 'convolutional':
                conv_layer = module_def[ 0 ]
                # If batch norm, load bn first
                if module_def[ 'batch_normalize' ]:
                    bn_layer = module[ 1 ]
                    bn_layer.bias.data.to_cpu().numpy().tofile( fp )
                    bn_layer.weight.data.to_cpu().numpy.tofile( fp )
                    bn_layer.running_mean.data.to_cpu().numpy().tofile( fp )
                    bn_layer.running_var.data.to_cpu().numpy().tofile( fp )
                else:
                    conv_layer.bias.data.to_cpu().numpy().tofile( fp )
                conv_layer.weight.data.to_cpu().numpy().tofile( fp )
        fp.close()